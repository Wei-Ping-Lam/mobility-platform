...
| Match-specific investment option | `InvestmentRecommendation.match_id`, `InvestmentRecommendation.intervention`, `InvestmentRecommendation.evidence_qualified` | Nondominated option for the identified match; evidence-qualified and exploratory classes remain separate | Integrated with strict city/match identity; no option is labeled an optimum |
