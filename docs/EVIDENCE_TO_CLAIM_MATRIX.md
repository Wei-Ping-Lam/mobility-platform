...
| Regional transfer-hub candidate | GTFS snapshot `cities[*].regional_hubs[*]` | Event-valid, connectivity-ranked parent station to investigate as a transfer candidate | Integrated for all 11 cities; no claim of parking, curb, fleet, layover, accessibility, or engineering feasibility |
| Match-day operating pattern | `traffic_strategy_plans[*].predicted_pattern`, `.prediction_reasons`, `.prediction_features`, `.actions` | Evidence-labeled broad strategy family selected without reading the official benchmark | Integrated for all 78 matches; nine distinct families emerge from service, access, walking, and regional-hub features |
| Official-plan family agreement | `traffic_strategy_plans[*].benchmark_pattern`, `.benchmark_agreement`, `.benchmark_source_url` | In-sample comparison between the independently generated broad family and the reviewed official-plan label | Integrated for all 11 cities; not holdout accuracy and not validation of exact hubs, fleet, windows, or controls |
| Event-bus scale screen | `traffic_strategy_plans[*].required_buses_per_hour_low/base/high`, `.single_hub_feasibility` | Unconstrained bus equivalents per hour needed to translate the residual access gap under stated capacity/load assumptions | Integrated as a scenario screen; not fleet procurement, delivered throughput, or roadway feasibility |
| Published traffic controls | Source-audit traffic-management snapshot | May be cited from its official source outside the normalized cross-city plan | Retained for provenance only; generated `traffic_strategy_plans` expose no city-specific named controls or approved curb locations |
...
| Portfolio priority screen | Representative-match access evidence plus evidence-qualified `InvestmentRecommendation` rows | Bottleneck-matched measure to validate first, with reason, owner, cost, lead time, and dependencies | Integrated; generic added frequency remains exploratory until a route-specific operating plan is assigned |
| Screening cost ratio | `InvestmentRecommendation.comparison_cost_base`, `InvestmentRecommendation.cost_per_passenger`, `InvestmentRecommendation.cost_basis` | Per-event operations plus reusable capital per assumed event use, divided by peak-hour capacity addressed | Not observed cost per rider; the numerator and denominator use different planning horizons, total project/event cost remains visible, and reuse is an explicit scenario assumption |
